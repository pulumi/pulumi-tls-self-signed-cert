...
export class ComponentAnalyzer {
    ...
    private analyzeType(type: ts.Type, schema: ComponentSchema, parentName: string): SchemaProperty {
        ...
        console.log(`\nAnalyzing type: ${typeString}`);
        
        // Handle arrays
        if (type.symbol?.name === "Array" || typeString.includes("[]")) {
            console.log('Processing as array type');
        ...
        if (typeString.includes('Input<') || typeString.includes('Output<') || typeString.includes('OutputInstance<')) {
            console.log('Processing as Input/Output type');
            const typeRef = type as ts.TypeReference;
            
            if (typeRef.typeArguments && typeRef.typeArguments.length > 0) {
                const innerType = typeRef.typeArguments[0];
                console.log(`Unwrapping to inner type: ${this.checker.typeToString(innerType)}`);
            ...
            if (match) {
                ...
                console.log(`Parsed inner type from string: ${innerTypeStr}`);
        ...
        console.log(`Defaulting to unknown for type: ${typeString}`);
        return { type: "unknown" };
    }
